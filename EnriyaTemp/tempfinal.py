...
first_details=['ID','NAME','MAJOR','GENDER','YEAR','STARTING_YEAR']
with open(filename,'a',newline='') as file:
    csvwriter=csv.writer(file)
    csvwriter.writerow(first_details)
def markAttendanceCSV(id, name, major, standing, year, starting_year):
    ...
    if is_file_empty:
        with open(csv_file_path, 'w', newline='') as f:
            ...
            writer = csv.writer(f)
            writer.writerow(['ID', 'Name', 'Major', 'Standing', 'Year', 'Starting_Year', 'Timestamp'])

    with open(csv_file_path, 'r') as file:
        reader = csv.reader(file)
        id_list = [row[0] for row in reader if row]
        ...
        for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
            ...
            if matches[matchIndex]:
                ...
                if counter == 0:
                    ...
                    counter = 1
                    modeType = 1
            if unknown_detected and counter == 0:
                cvzone.putTextRect(imgBackground, "Loading", (275, 400))
                cv2.imshow("Face Attendance", imgBackground)
                cv2.waitKey(1)
                counter = 1
